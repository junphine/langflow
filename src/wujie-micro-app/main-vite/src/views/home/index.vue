  ...
  <div class="home">
    <div class="tool">
      ...
      <div class="button-list">
        ...
        <el-tooltip placement="bottom">
          <template #content>预加载+预执行</template>
          <el-switch
           inline-prompt
            size="large"
            class="switch button-gap"
        ...
        <el-tooltip placement="bottom">
          <template #content>主应用为hash模式</template>
...
const preload  = ref(window.localStorage.getItem("preload") !== "false")
const degrade = ref( window.localStorage.getItem("degrade") === "true" || !window.Proxy || !window.CustomElementRegistry)
const disable = ref(!window.Proxy || !window.CustomElementRegistry)
onMounted(()=>{
  window.localStorage.setItem('main-item','123456')
})
watch( preload,
  (newVal:any, oldVal) => {
    console.log(newVal)
   window.localStorage.setItem("preload", newVal);
...
watch( degrade,
  (newVal:any, oldVal) => {
       console.log(newVal)
    window.localStorage.setItem("degrade", newVal);
    setTimeout(() => window.location.reload(), 1000);
  }
);
</script>

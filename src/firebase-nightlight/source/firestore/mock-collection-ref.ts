...
import { MockFieldPath, MockFieldValue, MockFirestoreContent, MockFirestoreQuery, MockDocumentPair } from "./mock-firestore-types";
...
export class MockCollectionRef implements firebase.firestore.CollectionReference {
    ...
    public get(): Promise<firebase.firestore.QuerySnapshot> {
        return this.proxy.loadAllDocuments(this.jsonPath_)
        .then(pairs=>{
            this.store_.content[this.path] = {};
            pairs.forEach(pair=>{
                this.store_.content[this.path][pair.id] = pair.doc;
            });            
            const query = new MockQuerySnapshot({

...
class MetaDataMappingPrompt:
    ...
    def init_table_mapping_prompt(self):
        ...
        table_mapping_prompt = '标准库中定义了如下表：\n| 表英文名 | 表注释 |\n'
    ...
    def table_doc_prompt(self,ods_table:str,only_table=False):
        external_table_mapping_prompt = ''
        if self.source_table_meta_data:
            tbl_info = self.source_table_meta_data[ods_table.upper()]
            if tbl_info:
                if 'info' in tbl_info:
                    comment =  tbl_info.get('info',{}).get('TABLE_COMMENT')
                    if comment:
                        external_table_mapping_prompt+=f'表名:{ods_table}，表注释:{comment}\n'
                if not only_table and 'fields' in tbl_info:
                    for fname,field in tbl_info.get('fields',{}).items():
                        comment = field.get('COLUMN_COMMENT')
                        if comment:
                            external_table_mapping_prompt+=f'字段名:{fname}，字段注释:{comment}\n'
    ...
    def table_mapping_question(self,ods_table:str):
        input_str = ods_table
        input_str = self.source_db.get_table_info([ods_table])
        input_str = input_str[40:]
        external_table_mapping_prompt = self.table_doc_prompt(ods_table,True)
        prompt = self.table_mapping_prompt+ f'\n\n业务表:\n{input_str}\n{external_table_mapping_prompt}\n从标准库中选出可以与业务表相映射的表，返回表英文名,表注释\n'
    ...
    def fields_mapping_question(self,ods_table:str|list,std_table:str|list):
        ...
        prompt = f'标准表：\n{std_table_mapping_prompt}\n业务表：\n{ods_table_mapping_prompt}\n{external_table_mapping_prompt}\n开始字段映射任务,从所给标准表中选出与业务表"{ods_table}"的各个字段可以映射的字段，若无匹配字段，则跳过。使用表格格式返回，表格包含列（业务表字段名，业务表字段的中文注释，标准表名，标准字段名，标准字段的中文注释）。\n'
        return dict(prompt=prompt)

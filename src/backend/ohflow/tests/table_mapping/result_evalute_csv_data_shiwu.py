...
if std_result_dataset:
    ...
    for ods_table,row2 in std_result_dataset.items():
        ...
        for field,m_fields in row2.items():
            ...
            if m_fields==[]:
                if m_std_tables ==[]:
                    match_id = ods_table+'.'+field
                    if match_id in meta_data.matched_ods_field_table_dict:
                        humans_data_cn = meta_data.matched_ods_field_table_dict[match_id]
                        item = dict(ods_dataset_cn=ods_table,ods_data_cn=field,std_dataset_cn="",std_data_cn="",humans_data_cn=humans_data_cn)
                        std_matched_dict[ods_table].append(item)
                        fn+=1
                else:
                    for m_std_table in m_std_tables:
            ...
            for m_pos_field in m_fields:
                ...
                if match_id in meta_data.matched_dict or match2_id in meta_data.matched_field_dict:
                    item["humans"] = 1
                    tp+=1
                else:
                    ...
                    if match_id in meta_data.matched_ods_field_table_dict:
                        fp+=1
                        item["humans_data_cn"] = meta_data.matched_ods_field_table_dict[match_id]
                    else:
                        if std_field==field or (std_field in field and len(std_field)>4):
                            tp+=1
                            pass
                        else:
                            inc+=1

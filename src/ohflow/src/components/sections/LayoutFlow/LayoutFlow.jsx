...
import FlowDetailsModal from "./FlowDetailsModal.jsx";
...
function saveToAPI(workflowObject, flow) {
  let nodesToSave = convertNodesToSaveableJSON(flow);
  //We will use this array to store the nodes in a format that we can save
  workflowObject.data = nodesToSave
  if(!workflowObject.uuid){  // is template
    workflowObject.id = null;
  }
  return fetch(api_base_url+"/workflows/" + (workflowObject.id? workflowObject.id: ""), {
    method: workflowObject.id? "PATCH": "POST",
    headers: {
...
function deleteFlowAPI(workflowObject) {
  return fetch(api_base_url+"/workflows/" + workflowObject.uuid, {
    method: "DELETE",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
...
const LayoutFlow = ({ displayOnly = false, flowId = 1, setActive = (x)=>{} }) => {
  ...
  const [saveModalIsDisplayed, setSaveModalIsDisplayed] = useState(false);

  //Displays "usaved changes" to the screen when set to true
  const [unsavedChanges, setUnsavedChanges] = useState(false);

  const onFlowSave = useCallback(async (params) => {
    if (rfInstance) {
      const flow = rfInstance.toObject();
      workflowObject.name = params.name
      workflowObject.description = params.description
      const updated = await saveToAPI(workflowObject,flow);
      if(updated['message']){
  ...
  }, [rfInstance,workflowObject]);

  const onFlowDelete = async (params) => {
    // todo
    if(params.isLocked){
      return '流程被作为模板，已经被锁定！请先解锁';
    }
    else{
      const updated = await deleteFlowAPI(params);
      if(updated['message']){
        return updated['message'];
      }
  ...
  let nodes_fetched = false
  //Load nodes and workflows from API
  useEffect(() => {
    // load node templates
    if(nodeTemplates.length==0 && !nodes_fetched){
      fetch(api_base_url+"/nodes/")
        ...
        .then((json) => {
          ...
          console.log(
            "Loading nodes from API {api_base_url}/tenant/1/nodes"
          );
        });
      nodes_fetched = true
    }
  ...
  //Load nodes and workflows from API
  useEffect(() => {
    fetch(api_base_url+"/workflows/"+flowId)
      .then((res) => res.json())
      .then((json) => {
        let data = json.data
        ...
        if(!displayOnly){
          setWorkflowObject(json);
        }
        setNodes([...initialNodes]);
        setEdges([...initialEdges]);
        //Note: this will get called twice in dev if React.Strict mode is turned on
        console.log(
          "Loading workflow from API {api_base_url}/workflow/{flowId}/"
        );
      });
  ...
  return (
    <div className="layoutflow">
        ...
        {displayOnly ? null : (
          <div style={{ padding: '4 8' }}>
            <Title order={4} style={{ padding: 4 }}>
              ...
              &nbsp; #{flowId} &nbsp;&nbsp; {workflowObject?.name}
            </Title>
            <Text
              size="sm"
              color="dimmed"
              style={{ paddingLeft: 69, marginTop: -5 }}
            >
      ...
      <div className="controls">
        {displayOnly ? null : <Button onClick={(e)=>setSaveModalIsDisplayed(true)}>Save</Button>}
      </div>
      {activeNode ? (
        <NodeDetailsModal
      ...
      <FlowDetailsModal
         opened={saveModalIsDisplayed && workflowObject!=null}
         activeNode={workflowObject}
         onClose={() => setSaveModalIsDisplayed(false)}

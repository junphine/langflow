...
import { MockDatabaseContent,MockComposite } from "./mock-database-types";
...
export class MockRef implements firebase.database.ThenableReference {
    ...
    constructor(options: MockRefOptions) {
        ...
        if (this.path_ === ".info") {
            let content = this.database_.content as MockComposite;
            if (!content[".info"]){
                content[".info"] = {
                    connected: true,
                    serverTimeOffset: 0
    ...
    on(
        ...
        this.refEmitterBindings_.push({
            ...
            type: eventType,
            unboundError: errorCallback as Function || null,

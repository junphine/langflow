...
const getHtml = (story) =>
  ...
  <body>
    <div id="container">
      <div id="content">
        <div id="title" class="title" >${story.title}</div>
        <div id="blurb" class="description" >${story.blurb}</div>
        <div id="stage">
          <div id="output"></div>
          <div id="input">
            <div id="textrow">
              <div id="cursor">${story.cursor}</div>
              <input id="text" class="input" autofocus ></input>
            </div>
...
$(document).ready(function () {
  ...
  const trueValue = (value, type) => {
    ...
    if (typeof trueValue === "string") {
      trueValue = getTextValue(trueValue);
      switch (type) {
        case "Number":
...
const downloadStory = async (story) => {
  ...
  const prism = await prismRes.text();

  const prismMarkdownRes = await fetch(
    "/prism/prism-markdown.js"
  );
  const prismMarkdown = await prismMarkdownRes.text();
  
  const cssUrl = await storage.ref(story.theme).getDownloadURL();
  const cssRes = await fetch(cssUrl);
  const cssText = await cssRes.text();

  const cssBase = await fetch(
    "/themes/base.css"
  );
  const cssBaseText = await cssBase.text()

  // Add files to directory and zip
  css.file("base.css", cssBaseText);
  css.file("style.css", cssText);
  js.file("jquery.min.js", jquery);
  js.file("prism.js", prism);
  js.file("prism-markdown.js", prismMarkdown);

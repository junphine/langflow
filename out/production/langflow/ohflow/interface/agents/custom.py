...
from langflow.interface.toolkits.custom import VectorStoreHealthToolkit
...
class VectorStoreAgent(CustomAgentExecutor):
    ...
    def from_toolkit_and_llm(cls, llm: BaseLanguageModel, vectorstoreinfo: VectorStoreInfo, **kwargs: Any):
        ...
        # modify@byron
        if 'health' in vectorstoreinfo.name or 'drug' in vectorstoreinfo.name or 'disease' in vectorstoreinfo.name:
            toolkit = VectorStoreHealthToolkit(vectorstore_info=vectorstoreinfo, llm=llm)
        elif 'people' in vectorstoreinfo.name or 'patient' in vectorstoreinfo.name:
            toolkit = VectorStoreHealthToolkit(vectorstore_info=vectorstoreinfo, llm=llm)
        else:
